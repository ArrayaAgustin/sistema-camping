const axios = require('axios');

const API_URL = 'http://localhost:3001';

async function testAdminLogin() {
  console.log('🧪 Probando login del usuario admin...\n');
  
  try {
    // Datos del admin
    const adminCredentials = {
      username: 'admin',
      password: 'admin123'
    };
    
    console.log('📤 Enviando credenciales:');
    console.log(`   Usuario: ${adminCredentials.username}`);
    console.log(`   Password: ${adminCredentials.password}\n`);
    
    // Hacer petición POST al endpoint de login
    const response = await axios.post(`${API_URL}/auth/login`, adminCredentials, {
      headers: {
        'Content-Type': 'application/json'
      }
    });
    
    console.log('✅ Login exitoso!');
    console.log('📄 Respuesta del servidor:');
    console.log('   Status:', response.status);
    console.log('   Token:', response.data.token ? 'Generado ✅' : 'No generado ❌');
    console.log('   Usuario:', response.data.user);
    
    if (response.data.user) {
      console.log('\n👤 Datos del usuario:');
      console.log(`   ID: ${response.data.user.id}`);
      console.log(`   Username: ${response.data.user.username}`);
      console.log(`   Afiliado ID: ${response.data.user.afiliado_id || 'No asignado'}`);
      console.log(`   Roles: ${response.data.user.roles ? JSON.stringify(response.data.user.roles) : 'No roles'}`);
      console.log(`   Permisos: ${response.data.user.permisos ? JSON.stringify(response.data.user.permisos) : 'No permisos'}`);
    }
    
    // Guardar token para futuras pruebas
    if (response.data.token) {
      console.log('\n🎫 Token JWT (primeros 50 caracteres):');
      console.log(`   ${response.data.token.substring(0, 50)}...`);
    }
    
  } catch (error) {
    console.log('❌ Error en el login:');
    
    if (error.response) {
      // El servidor respondió con un código de error
      console.log('   Status:', error.response.status);
      console.log('   Error:', error.response.data);
      
      if (error.response.status === 401) {
        console.log('\n💡 Posibles causas:');
        console.log('   - La contraseña no coincide con el hash en la DB');
        console.log('   - El usuario no existe');
        console.log('   - El usuario está inactivo');
      }
      
    } else if (error.request) {
      // No hubo respuesta del servidor
      console.log('   No se pudo conectar al servidor');
      console.log('   ¿Está ejecutándose en puerto 3001?');
      
    } else {
      // Error en la configuración de la petición
      console.log('   Error:', error.message);
    }
  }
}

// Ejecutar test
testAdminLogin();